...
for p in range(1, 55):
    ...
    cells[0].text = "Election and Candidate"
    
    for i in range(6): cells[i+1].text = f"{races[i]},{str(dems[i][0])}"
    cells = table.rows[3].cells
    cells[0].text = "Precinct Turnout"
    for i in range(6): cells[i+1].text = str( round(prec_turnouts[p][i], 2))
    cells = table.rows[4].cells
    cells[0].text = "Arlington Turnout"
    for i in range(6): cells[i+1].text = str( real_turnouts[i])
    cells = table.rows[5].cells
    cells[0].text = "Precinct Dem"
    for i in range(6): cells[i+1].text = str(round(precinct_code_to_votes_dem[p][i]/(precinct_code_to_votes_dem[p][i]+precinct_code_to_votes_rep[p][i]), 2))
    cells = table.rows[6].cells
    cells[0].text = "Arlington Dem"
    for i in range(6): cells[i+1].text = str(round(perc_arl_dem[i], 2))
    plt.plot([2017, 2018, 2019, 2020, 2021, 2022], [prec_turnouts[p][i] for i in range(6)], label = "Precinct Turnout")
    plt.plot([2017, 2018, 2019, 2020, 2021, 2022], [real_turnouts[i] for i in range(6)], label = "Arlington Turnout")
    plt.plot([2017, 2018, 2019, 2020, 2021, 2022], [precinct_code_to_votes_dem[p][i]/(precinct_code_to_votes_dem[p][i]+precinct_code_to_votes_rep[p][i]) for i in range(6)], label = "Precinct Dem")
    plt.plot([2017, 2018, 2019, 2020, 2021, 2022], [perc_arl_dem[i] for i in range(6)], label = "Arlington Dem")
